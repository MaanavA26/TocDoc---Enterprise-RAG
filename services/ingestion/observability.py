...
This file is duplicated at `services/qna/src/core/observability.py` with
...
def _ensure_handler_attached(logger: logging.Logger) -> None:
    ...
    The ingestion app installs handlers via `logging.basicConfig` at module
    import time, which DOES configure root — so the safeguard usually no-ops
    in this service. We keep it for parity with the qna copy, and to defend
    against import-order changes that might leave the root logger unconfigured.
...
class RequestIDMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            # Re-raise — let Starlette's ServerErrorMiddleware generate the 500
            # response. We deliberately do NOT own the global unhandled-exception
            # response here; that responsibility belongs to the structured
            # error-contract layer (P0-6, separate workstream).
            #
            # Known limitation (deferred to P0-6): 500 responses generated by
            # ServerErrorMiddleware will NOT carry X-Request-ID. Responses
            # produced via HTTPException (4xx and the more common 5xx path)
            # DO carry it, because FastAPI's ExceptionMiddleware returns a
            # Response that this middleware sees on its way out and decorates
            # with the header. P0-6 will close the unhandled-exception gap by
            # introducing a dedicated error-contract handler.
            raise
